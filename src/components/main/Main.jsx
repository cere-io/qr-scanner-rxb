...
const Main = () => {
  ...
  function onEngagementAction(template) {
    ...
    setTimeout(() => {
      ...
      doc.addEventListener(
        'ITEM_CLAIMED',
        (e) => console.log(e.detail), // outputs: {foo: 'bar'}
        false,
      );
      doc.close();
    }, 100);
  ...
  function sendEvent(sdk, data) {
    ...
    // @ts-ignore
    sdk?.sendEvent('DAVINCI_QR_CODE_VALIDATOR', request);
  }
  ...
  return (
    <div className="header">
      ...
      {showQr && !showUserCreds && (
        <div className="modal">
          <div className="modal-content1">
            <span className="close-button" onClick={() => setShowQr(false)}>
              &times;
            </span>
            <h2>QR Scanner</h2>
            <QrReader
      ...
      {show && !showQr && (
        <div className="modal">
          <div className="modal-content1">
            ...
            <iframe id="contentIFrame" width={330} height={500} frameBorder={0} title="Iframe title"></iframe>
          </div>
        </div>
      )}
      ...
      {showUserCreds && (
        <div className="modal">
          <div className="modal-content1 border border-red-600    ">
            <h2>Login </h2>
            ...
            <div align={'center'} color={'black'}>
              <Button type="button" onClick={applyUserData}>
                Apply
              </Button>
            </div>
          </div>
      ...
      {getEventId() && (
        <div className="navbar-sign" style={{paddingBottom: 50}}>
          ...
          <Button onClick={logout}>
            Log out {userData}/{location}/{event}
          </Button>
        </div>
      )}
    </div>
